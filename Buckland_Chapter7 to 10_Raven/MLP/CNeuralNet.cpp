...
//
//---------------------------------------------------------------------
SNeuron::SNeuron(int NumInputs = 4): m_iNumInputs(NumInputs+1),
                                 m_dActivation(0),
                                 m_dError(0)
...
void SNeuron::SaveNeuron(FILE * file) const {
	fwrite(&m_iNumInputs, sizeof(m_iNumInputs), 1, file);
	fwrite(&m_vecWeight[0], sizeof(m_vecWeight[0]), m_iNumInputs, file);
	fwrite(&m_dActivation, sizeof(m_dActivation), 1, file);
	fwrite(&m_dError, sizeof(m_dError), 1, file);
};
void SNeuron::LoadNeuron(FILE * file) {
	m_vecWeight.clear();

	fread(&m_iNumInputs, sizeof(m_iNumInputs), 1, file);
	m_vecWeight.resize(m_iNumInputs);
	fread(&m_vecWeight[0], sizeof(m_vecWeight[0]), m_iNumInputs, file);
	fread(&m_dActivation, sizeof(m_dActivation), 1, file);
	fread(&m_dError, sizeof(m_dError), 1, file);
};
...
//-----------------------------------------------------------------------
SNeuronLayer::SNeuronLayer(int NumNeurons = 4, 
...
void SNeuronLayer::SaveLayer(FILE * file) const {
	fwrite(&m_iNumNeurons, sizeof(m_iNumNeurons), 1, file);

	for (size_t i = 0; i < m_iNumNeurons; i++) {
		m_vecNeurons[i].SaveNeuron(file);
	}
};
void SNeuronLayer::LoadLayer(FILE * file) {
	m_vecNeurons.clear();

	fread(&m_iNumNeurons, sizeof(m_iNumNeurons), 1, file);

	m_vecNeurons.resize(m_iNumNeurons);
	for (size_t i = 0; i < m_iNumNeurons; i++) {
		m_vecNeurons[i].LoadNeuron(file);
	}
...
void CNeuralNet::SaveMLPNetwork(const std::string & filename)const {
	FILE * file;
	file = fopen(filename.c_str(), "wb");
	fwrite(&m_iNumInputs, sizeof(m_iNumInputs), 1, file);
	fwrite(&m_iNumOutputs, sizeof(m_iNumOutputs), 1, file);
